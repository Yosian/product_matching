...
def import_data():
    temp = pd.read_csv('train.tsv', sep='\t', index_col='train_id', nrows=400000,
...
class GenerateModel:
    ...
    def _prepare_text(self) -> None:
        if self.tokeniser is None:
            self.tokeniser = Tokenizer(num_words=Parameters.vocab_size, oov_token='<OOV>')
        self.tokeniser.fit_on_texts(self.frame['cleaned_text'])
        if self.sequences is None:
            self.sequences = self.tokeniser.texts_to_sequences(self.frame['cleaned_text'])
        if self.padded_sequences is None:
            self.padded_sequences = pad_sequences(self.sequences,
                                                  maxlen=Parameters.max_len,
                                                  padding='post', truncating='post')

    def generate(self) -> keras.engine.sequential.Sequential:
        self._prepare_text()
        self.model = Sequential([
            Embedding(Parameters.vocab_size,
                      Parameters.embedding_dim,
                      input_length=Parameters.max_len),
            GlobalAveragePooling1D(), Dense(16, activation='relu'), Dropout(0.5), Dense(1)
        ])

        self.model.compile(optimizer='adam', loss='mse')
        self.model.fit(self.padded_sequences, self.padded_sequences,
